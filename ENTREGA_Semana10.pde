...
void mousePressed() {
  ...
  if (mouseX>60&&mouseX<90&&mouseY>150&&mouseY<height-30) {
    dragS=true;
    s=map(mouseY, 150, height-30, 0, 255);
    ui=true;
  }
  ...
  if (herramienta==2) {
    for (int i=0; i<8; i++) {
      if (dist(mouseX, mouseY, 65+4*90+i*70, 70)<35) {
        forma=i;
        ui=true;
      }
    }
...
class Pincelada {
  ...
  Pincelada(float x, float y, color c) {
    this.x = x;
    this.y = y;
    this.c = c;
  }
